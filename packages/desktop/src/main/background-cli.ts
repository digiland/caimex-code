...
// Every app id this desktop has shipped under. The Caimex ids come first
// because they are the ones in use; the OpenCode-branded ones are kept so a
// daemon already running for a pre-rename install is adopted rather than
// duplicated. Keep in step with APP_IDS in electron-builder.config.ts.
...
export async function startBackgroundCli(logger: Logger, shellStateHome?: string) {
  ...
  const bundled =
    overridden ??
    (app.isPackaged ? join(process.resourcesPath, executableName()) : join(root, "../../resources", executableName()))
...
// this fork's v2 source still exposes a bare `service password`. Neither is
// wrong, and the desktop has to drive whichever binary it was pointed at — so
// try the newer form and fall back rather than pinning to one.
//
